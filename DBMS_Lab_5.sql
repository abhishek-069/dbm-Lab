...
-- A. SELECT DISTINCT S.SNAME FROM STUDENT S JOIN ENROLLED E ON S.SNUM = E.SNUM JOIN CLASS C ON E.CNAME = C.CNAME JOIN FACULTY F ON C.FID = F.FID WHERE S.LEV = 'JR' AND F.FNAME = 'RAKESH';

 B) Find the age of the oldest student who is either a history major or enrolled in a course taught by Ravi.
-- B. SELECT MAX(S.AGE) FROM STUDENT S WHERE S.MAJOR = 'HISTORY' OR S.SNUM IN (SELECT E.SNUM FROM ENROLLED E JOIN CLASS C ON E.CNAME = C.CNAME JOIN FACULTY F ON C.FID = F.FID WHERE F.FNAME = 'RAVI');

 C) Find the names of all students who are enrolled in two classes that meet at the same time. 
-- C. SELECT DISTINCT S.SNAME FROM STUDENT S JOIN ENROLLED E1 ON S.SNUM = E1.SNUM JOIN ENROLLED E2 ON S.SNUM = E2.SNUM JOIN CLASS C1 ON E1.CNAME = C1.CNAME JOIN CLASS C2 ON E2.CNAME = C2.CNAME WHERE C1.MEETS_AT = C2.MEETS_AT AND C1.CNAME <> C2.CNAME;
...
-- D. SELECT F.FNAME, COUNT(C.CNAME) AS TOTALCLASSES FROM FACULTY F JOIN CLASS C ON F.FID = C.FID WHERE F.FID NOT IN (SELECT DISTINCT C2.FID FROM CLASS C2 WHERE C2.ROOM <> 'R128') GROUP BY F.FNAME;

 E) Create a view that contains the details of students along with the name of the courses enrolled.
-- E. CREATE VIEW STUDENTCOURSEDETAILS AS SELECT S.SNUM, S.SNAME, S.MAJOR, S.LEV, S.AGE, C.CNAME AS COURSENAME FROM STUDENT S JOIN ENROLLED E ON S.SNUM = E.SNUM JOIN CLASS C ON E.CNAME = C.CNAME;
